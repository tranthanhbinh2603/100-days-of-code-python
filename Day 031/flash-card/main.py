...
def wrong():
    ...
    global choosed
    choosed = randint(0, len(data['French']) - 1)
    canvas.itemconfig(language_text, text="French")
    canvas.itemconfig(word_text, text=data['French'][choosed])
...
def right():
    ...
    global choosed
    canvas.itemconfig(language_text, text="French")
    canvas.itemconfig(word_text, text=text=data[0]['Franch'])
    canvas.itemconfig(image_canvas, image=image_front)
    windows_after = scrren.after(3000, change_image)
...
def change_image():
    ...
    canvas.itemconfig(word_text, text=data['English'][choosed])
...
#choosed = randint(0, len(data['French']) - 1)
word_text = canvas.create_text(400, 263, text=data[0]['Franch'], font=FONT_WORD)
